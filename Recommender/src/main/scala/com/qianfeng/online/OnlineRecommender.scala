...
object ConnHelper extends Serializable{
  // 懒变量定义，使用的时候才初始化
  lazy val jedis = new Jedis("qianfeng01")
  jedis.auth("123456")
  lazy val mongoClient = MongoClient(MongoClientURI("mongodb://192.168.10.101:27017/recommender"))
}
...
object OnlineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    val config = Map(
      "spark.cores" -> "local[*]",
      "mongo.uri" -> "mongodb://192.168.10.101:27017/recommender",
    ...
    scores
      ...
      .map{
        case (productId, scoreList) =>
          ( productId, scoreList.map(_._2).sum / scoreList.length   //平均分
